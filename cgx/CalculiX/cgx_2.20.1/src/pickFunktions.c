...
{
  int i,j,l,lmaster,n,s;
  int nlines, nls[2], linoffs;
  int snew, sbuf, Stmp, sh_buf, nurbsnr, patch;
  char repFlag=0;
  ...
  double p1[3],p2[3],p1p2[3], p1pn[3],vn[3],vl,pnt[5],ubuf,vbuf;
  double surfcg_uvw[3], lineLoop_uvw[1000][3],p1scg[3],p1lcg[3];

      printf("ptr_linebuf:%d\n",*ptr_lmaster);
      printf("ptr_linebuf:%d\n",ptr_lmaster[0]);
  
  printf(" surf %s  nc:%d nl0:%d\n", surf[smaster].name, surf[smaster].nc,surf[smaster].c[0]);
  ...
  // TBD, currently the splitting works only with just one lmaster
  if(nl>1) { printf(" ERROR: currently the splitting works only with just one lmaster\n"); return(-5); }
  lmaster=ptr_lmaster[0];
  ...
  /* add the inner line-loops to the surf-lines */

  /* first get the uv average of the line loops and later compare to the average of the 2 surfs. 
  ...
  if(surf[smaster].nc>1)
  {
	...
	// eval the uv coords of the splitting line
        n=line[lmaster].p1;
        pnt[0]=point[n].px;
        pnt[1]=point[n].py;
        pnt[2]=point[n].pz;
	...
	// cg of potential new first surface left or right from splitline
	pnt[0]=pnt[1]=pnt[2]=0.;
	for(l=0; l<nls[0]; l++)
	{
	  pnt[0]+=point[line[ls[0][l]].p1].px+point[line[ls[0][l]].p2].px;
	  pnt[1]+=point[line[ls[0][l]].p1].py+point[line[ls[0][l]].p2].py;
	  pnt[2]+=point[line[ls[0][l]].p1].pz+point[line[ls[0][l]].p2].pz;
	}
	pnt[0]/=nls[0]*2.;
	pnt[1]/=nls[0]*2.;
	pnt[2]/=nls[0]*2.;
	...
	surfcg_uvw[1]=pnt[4];
	surfcg_uvw[2]=0.;
	v_result(p1,&surfcg_uvw[0],p1scg);
	v_prod(p1p2,p1scg,vn);
	surfcg_uvw[2]=vn[2];

        // determine the uv coordinate of the cg of nls[0] in uv to determine which surf is at the same side as the loops
	//ls[0]
        //proj1PntToNurbs( nurbsnr, pnt);
        linoffs=0;  
	  ...
	  // inner loops left or right from splitline
  	  v_result(p1,&lineLoop_uvw[i][0],p1lcg);
	  v_prod(p1p2,p1lcg,vn);
	  lineLoop_uvw[i][2]=vn[2];
	  //printf("loop:%d uvw:%f %f %f\n",i,lineLoop_uvw[i][0],lineLoop_uvw[i][1],lineLoop_uvw[i][2]);
	  
	  // which surf belongs to left or right?	  
  	  // add the line loops to the line buffers of the new surfaces (TBD)
	  if((surfcg_uvw[2]*lineLoop_uvw[i][2])>0.)
	  {
...
{
  int i,ii,j,jj,k,e,ee,n,nn,nset,eset,anz_n,anz_nmax,anz_e,anz_nset,nel[3];
  ...
  double cutParam_x,cutParam_y,cutParam_z;  
