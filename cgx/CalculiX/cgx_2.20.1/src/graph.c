...
extern int       cur_lc;                                      /* pre-selected Dataset, active after entity is selected */
...
void plot2D(char *type, int setNr, int *dsNr, int entity )
{
  ...
  for(i=1; i<=dsNr[0]; i++)
  {
    ...
    if((lc_step=(int *)realloc(lc_step, (anz_lc+2) *sizeof(int)))==NULL )
      printf("\n\n ERROR: realloc failed \n\n") ;
    ...
    if(j<lcase[lc].npheader);
    else if((type[0]=='s')||(type[0]=='n')||(type[0]=='f')||(type[0]=='t')||(type[0]=='d'))
    {
      lc_step[anz_lc] = lcase[lc].step_number;
      lc_time[anz_lc] = lcase[lc].value;
    }
    else
    {
      printf(" ERROR: could not find parameter:%s\n", type);
    ...
    if (!lcase[lc].loaded)
    {
      ...
      if(copiedNodeSets[0].sets)
      {
	// copy Datasets to copied nodes (slow, since all nodes of the dataset are loaded)
...
*/
  ...
  else if((type[0]=='T')||(type[0]=='f'))
    fprintf (handle_out, "FREQUENCY INCREMENT / NODE\n");
  ...
  for (lc=0; lc<anz_lc; lc++)
  {
    ...
    fprintf (handle_out, "%-5d %-5d %-12.5e %f ", lc_step[lc], dsNr[lc+1]+1, lc_time[lc], atof(lc_description[lc]));
    for ( n=0; n<set[setNr].anz_n; n++)
...
int graph( char *record)
{
  ...
  lcmin--; lcmax--;
  if(!anz->l) { printf(" ERROR: No datasets available\n"); return(-1); }
  if(lcmax<0) lcmax=anz->l-1;
  if(lcmax>anz->l) lcmax=anz->l-1;
  if(lcmin<0) lcmin=0;
  ...
  if (compare(type, "length", 1)==1)
  {
	...
	{
          areampc(n, j, "map", "surf", buffer, 0, 0, node, 0, 1);
    ...
    else if((set[indx].anz_n>0)&&(set[indx].type==0))
    {
      /* open a node set */
      /* generate a name based on the setname */
      sprintf(name,"N%s",buffer);
     ...
     maxdist=0.;
      ...
      if(j<0) printf("ERROR:set all does not exist\n");
      n=interpol3d(seq, j, "vol", buffer, 0);
      if(n>0) areampc(n, j, "map", "surf", buffer, 0, 0, node, 0, 1);
      zap(specialset->nompc);
      */
    }
    else if((set[indx].anz_n>0)&&(set[indx].type>0))
    ...
    if((type[0]=='c')||(type[1]=='c')); // just keep the sequence
    else
    {
...
//   if((inpformat)&&(graph_on))   sprintf(buffer, "%s amplitude.png &", psviewer );
    ...
    sprintf(buffer, "gnuplot amplitude.gnu" );
    system (buffer);
    if((inpformat)&&(graph_on))  sprintf(buffer, "%s amplitude.ps &", psviewer );
    system (buffer);
#endif
    free(dsNr);
  } 
  ...
  else if (compare(type, "mat", 1)==1)
  {
    ...
    for (nr=0; nr<anz->mats; nr++)
    {
      ...
      if(material[indx].npl)
      {
        sprintf(buffer, "%s_plastic.out",material[nr].name);
        handle_out = fopen (buffer, "w+b" );
        if (handle_out==NULL) { printf ("\nThe output file \"%s\" could not be opened.\n\n", buffer); return(-1);}
	buf=material[indx].tpl[0];
        for (i=0; i<material[indx].npl; i++)
        {
	  if(buf!= material[indx].tpl[i]) fprintf (handle_out, "\n\n");
          fprintf (handle_out, "%d %e %e %e\n", i+1,material[indx].spl[i],material[indx].epl[i], material[indx].tpl[i]);
	  buf=material[indx].tpl[i];
        }
        fclose(handle_out);
      }
      if((dsNr=(int *)realloc((int *)dsNr, (anz_lc+1) *sizeof(int)))==NULL )
    ...
    for (nr=0; nr<anz_lc; nr++)
    {
      if(material[dsNr[nr]].nela)
      {
        ...
        if(sum) fprintf(handle_gnu, ", \"%s\" using 4:2 title '%s' with linespoints\\\n", buffer, material[dsNr[nr]].name);
    ...
    for (nr=0; nr<anz_lc; nr++)
    {
      if(material[dsNr[nr]].nexp)
      {
        ...
        if(sum) fprintf(handle_gnu, ", \"%s\" using 3:2 title '%s' with linespoints\\\n", buffer, material[dsNr[nr]].name);
    ...
    fprintf(handle_gnu, "set ylabel \" Stress \"\n");
    fprintf(handle_gnu, "set xlabel \" Plastic Strain \"\n");
    for (nr=0; nr<anz_lc; nr++)
    {
      if(material[dsNr[nr]].npl)
      {
        sprintf(buffer, "%s_plastic.out", material[dsNr[nr]].name);
        fprintf(handle_gnu, "set title \"%s\"\n", material[dsNr[nr]].name);
	buf=-1.;
        sum=0;
        for (i=0; i<material[dsNr[nr]].npl; i++)
        {
	  // new temps
	  if(buf!= material[dsNr[nr]].tpl[i])
	  {
            buf=material[dsNr[nr]].tpl[i];
            if(sum) fprintf(handle_gnu, ", \"%s\" index %d using 3:2 title '%s T:%.1f' with linespoints\\\n", buffer, sum, material[dsNr[nr]].name, buf);
	    else fprintf(handle_gnu, "plot \"%s\" index %d using 3:2 title '%s T:%.1f' with linespoints\\\n", buffer, sum, material[dsNr[nr]].name, buf);
            sum++;
	  }
	}
        if(sum) fprintf(handle_gnu, "\n");
...
//   if((inpformat)&&(graph_on))   sprintf(buffer, "%s material.png &", psviewer );
    ...
    sprintf(buffer, "gnuplot material.gnu" );
    system (buffer);
    if((inpformat)&&(graph_on))  sprintf(buffer, "%s material.ps &", psviewer );
    system (buffer);
#endif
    free(dsNr);
  } 
