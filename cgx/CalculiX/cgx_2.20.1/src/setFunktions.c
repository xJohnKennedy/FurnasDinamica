...
{
  int  c,e,i,j=0,k,kk,n,nn,m,p,ll,l,d1,d2,ss,s, setNr, trgtNr, trkNr, mode, ibuf=0, orient=0;
    ...
    for (ss=0; ss<set[setNr].anz_s; ss++)
    {
  	...
  	do
  	{
    	  ...
    	  if(n2==-1) break;
            ...
            if(dist2<distn)
            {
      	    if(printFlag) printf(" found endpoint\n");
  		...
  		printf("loop %d, max pos change:%f, tol:%f\n",ii,dist2,ltol*0.5);
  		if(dist2<ltol*0.5) break;
  	      }
