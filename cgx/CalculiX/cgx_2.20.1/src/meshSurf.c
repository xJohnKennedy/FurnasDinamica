...
int mesh_tr3u(int nr, int renderFlag)
{
  ...
  if(surf[nr].fail==2)
  {
    printf(" ERROR: tr3u requires a trimmed nurbs. nurbs not trimmed for surf:%s\n",surf[nr].name);  return(-1);
  }

  /* the uv patch was created in meshSurfs() */
  ...
  int  sum_div;
    ...
    if (surf[j].nl!=MAX_EDGES_PER_SURF)
    {
      if (surf[j].nl==2)
      {
        ...
        surf[s].permElem=surf[j].permElem;
...
int meshSurf( int anz_s, int sindx, int snr, int *mapsurf, int meshflag, int renderFlag)
{
  ...
  /* evaluate the surf */
    ...
    if(surf[j].o==NULL) { returnVal=-2; return(returnVal); }
  ...
  printf("hallo3 unbalanced:%d\n",jbuf);
      ...
      // if no shell elements
      if((surf[j].etyp<7)||(renderFlag))
      {
	// proj to embeded nurbs 
    ...
    sem_wait(&sem_nurs);
  	    Stmp=-1;
  	    if((shape[surf[j].sh].type==1)||(shape[surf[j].sh].type==2)) Stmp=coneToNurs(surf[j].sh, 0);
  	    if(shape[surf[j].sh].type==3) Stmp=sphToNurs(surf[j].sh, 0);
  	    if(shape[surf[j].sh].type==5) Stmp=torusToNurs(surf[j].sh, 0);
  	    if(Stmp>-1)
  	    {
              projSurfToNurbs( Stmp, surf, j, &npre );
              for (n=0; n<nurbs[Stmp].u_npnt; n++) delPnt( nurbs[Stmp].v_npnt, nurbs[Stmp].ctlpnt[n] );
              //seta( setall, "S", Stmp );
              delNurs( 1, &Stmp );
  	    }
    sem_post(&sem_nurs);
    ...
    sem_post(&sem_n);
        ...
        for (b=0; b<div_b  ; b++)
        {
          for (a=0; a<div_a; a++)
          {
            if ((a>=div_l[sa1])&&(b<offs_sa1)) goto noelem7a;
            if ((a>=div_l[sa2])&&(b>=div_b-offs_sa2)) goto noelem7a;
            if (surf[j].ori=='+')
            {
              ebuf[0]=n_ba[(b  )*amax + a    ];
              ebuf[1]=n_ba[(b+1)*amax + a    ];
  ...
  printf("hallo qu8 unbalanced:%d\n",jbuf);
    ...
    sem_post(&sem_n);
      ...
      if(( nurbsflag )&&(surf[j].etyp>6)&&( surf[j].sh>-1 )&&(!renderFlag))
      {
        if(shape[surf[j].sh].type==4) projSurfToNurbs( shape[surf[j].sh].p[0], surf, j, &npre );
        else
	{
  ...
  printf("hallo before:%d\n",jbuf);
    ...
    sem_post(&sem_n);
      ...
      else if ((surf[j].etyp==7)&&(surf[j].permElem==1))
      {
        /* allocate memory for embeded elements */
    ...
    sem_post(&sem_n);
      ...
      else if ((surf[j].etyp==8)&&(surf[j].permElem==1))
      {
        /* allocate memory for embeded elements */
        if((surf[j].elem=(int *)realloc((int *)surf[j].elem, (div_l[0]*div_l[3]/2)*sizeof(int)) )==NULL)
    ...
    sem_post(&sem_n);
      ...
      else if ((surf[j].etyp==9)&&(surf[j].permElem==1))
      {
        /* allocate memory for embeded elements */
    ...
    sem_post(&sem_n);
      ...
      else if ((surf[j].etyp==10)&&(surf[j].permElem==1))
      {
#if TEST2
  printf("hallo etyp:%d\n",jbuf);
#endif
