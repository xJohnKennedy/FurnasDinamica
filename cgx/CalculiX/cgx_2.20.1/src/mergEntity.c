...
int mergeNode( int *arrayIndx, int anz_n, int indx, int lock, int *nodes, int **nod2elem )
{
  ...
  int anz_qcut=0, setQcut;

  setQcut=getSetNr("-qcut");
  if(setQcut>-1) anz_qcut=set[setQcut].anz_n;
  ...
  for (i=1; i<=nodes[0]; i++)
  {
    ...
    if (arrayIndx[nodes[i]] > 0)
    {
      ...
      // the nodes in the temporary set "-qcut" need special care
      if(setQcut > -1) if(getIndex(&set[setQcut].node,set[setQcut].anz_n,nod2) > -1)
      {
	for(j=0; j<anz_qcut; j++)
	{
	  if(qcut_nod[j].nr==nod2)
	  {
	    for(k=j; k<anz_qcut-1; k++)
	    {
	      qcut_nod[k].nr=qcut_nod[k+1].nr;
	      qcut_nod[k].n1=qcut_nod[k+1].n1;
	      qcut_nod[k].n2=qcut_nod[k+1].n2;
	      qcut_nod[k].val=qcut_nod[k+1].val;
	    }
	    anz_qcut--;
	  }
	}
...
void mergeElem( int setNr, int delFlag )
{
  ...
  {
    for(i=0; i<sum_delem; i++) { setr( setNr, "e", delem[i]); e_enqire[delem[i]].type*=-1; }
    //printf("%d elems from set:%s removed and deactivated\n", sum_delem, set[setNr].name);
  }
...
int mergePnt( Rsort *rsort, int anz_p, int indx, double local_gtol, int lock )
{
  ...
  for (i=1; i<=pnts[0]; i++)
  {
    if (rsort[pnts[i]].i > -1)
    {
      ...
      /* change point in shapes if that point is used  */
      for (j=0; j<anzGeo->sh; j++)
      {
        if( shape[j].name != (char *)NULL)
        {
          if(shape[j].type==3) for (n=0; n<7; n++) { if (shape[j].p[n] == pnt2) shape[j].p[n]=pnt; }
          if((shape[j].type==0)||(shape[j].type==1)) for (n=0; n<3; n++) { if (shape[j].p[n] == pnt2) shape[j].p[n]=pnt; }
          if((shape[j].type==2)||(shape[j].type==5)) for (n=0; n<4; n++) { if (shape[j].p[n] == pnt2) shape[j].p[n]=pnt; }
        }
      }
   ...
   if(( line[i].name != (char *)NULL ) && (i!=indx))
   {
         ...
         ((line[indx].p1 == line[i].p2)&&(line[indx].p2 == line[i].p1)) )
    {
      ...
      if(((line[indx].trk == line[i].trk)||(equalFlag))||(!lock))
      {
       ...
       for (j=0; j<anzGeo->c; j++)
       {
        if( lcmb[j].name != (char *)NULL) for (k=0; k<lcmb[j].nl; k++)
        {
          if (lcmb[j].l[k] == indx)
          {
      ...
      if (delflag)
      {
        ...
        {
          if( surf[j].name != (char *)NULL) for (k=0; k<surf[j].nl; k++)
          {
    ...
    if(( lcmb[i].name != (char *)NULL) && (i!=indx) )
    {
         ...
         ((lcmb[indx].p1 == lcmb[i].p2)&&(lcmb[indx].p2 == lcmb[i].p1)) )
      {
        ...
        for (j=0; j<anzGeo->s; j++) if( surf[j].name != (char *)NULL)
        {
          ...
          {
            if ((surf[j].l[k] == indx)&&(surf[j].typ[k]=='c'))
            {
...
int mergeSurf( int indx, double local_gtol )
{
  ...
  for (i=0; i<anzGeo->s; i++)
  {
    ...
    for (j=0; j<anzGeo->b; j++)
    {
      ...
      {
        if (( body[j].name != (char *)NULL )&&(body[j].s[k] == indx))
        {
    ...
    for (j=0; j<anzGeo->b; j++) if( body[j].name != (char *)NULL )
    {
      for (k=0; k<body[j].ns; k++)
      {
        if (body[j].s[k] == i)
...
void pre_merge( char *record )
{
  ...
  int     n_closest_nodes=0, ipuf=0, counter;
  ...
  printf(" merge ongoing, please wait\n");
  ...
  if (( type[0]=='n' )||( type[0]=='N' ))
  {
    ...
    if ( (arrayIndx = (int *)malloc( (anz->nmax) * sizeof(int))) == NULL )
      printf("ERROR: malloc failed: Rsort\n\n" ); 
    ...
    for(i=0; i<k; i++)
    {
      ...
      n_closest_nodes=10;
      if(i>counter) { printf(" evaluated:%d from:%d\n", i, k); counter*=10; }
    repeatNodSearch:;
      ...
      ipuf=n_closest_nodes;
      //for(j=1; j<=n_closest_nodes; j++) printf("near_node j:%d n:%d\n",j,near_node[j]);
      if(n_closest_nodes<k-1)
      {
        // arrayIndx[near_node[n_closest_nodes]]==nodeNr, if deleted set to 0 in mergNode()
        ipuf=n_closest_nodes;
        while(arrayIndx[near_node[ipuf]]==0) { ipuf--; }
        dx=node[arrayIndx[i]].nx-node[arrayIndx[near_node[ipuf]]].nx;
        dy=node[arrayIndx[i]].ny-node[arrayIndx[near_node[ipuf]]].ny;
        dz=node[arrayIndx[i]].nz-node[arrayIndx[near_node[ipuf]]].nz;
        if((abs(dx)<=local_gtol) &&(abs(dy)<=local_gtol) &&(abs(dz)<=local_gtol)) { n_closest_nodes*=2; goto repeatNodSearch; }
      }
      n=near_node[0]=ipuf;
      for(j=ipuf; j>0; j--)
      {
	//printf("nmax:%d near_node[%d]:%d\n",anz->nmax,j,near_node[j]);
        if(arrayIndx[near_node[j]]==0) { n--; near_node[j]=-1; continue; }
      ...
      if(n)
      {
        if(printFlag) printf("merge %d from:%d\n", i+1, k); 
